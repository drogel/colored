...
const testColorHexesApiResponse = {
  "apiVersion": "0.2.1",
  "method": "GET",
  "data": {
    "kind": "color",
...
void main() {
  ...
  group("Given a $ColorHexesApiService with a null apiIndex", () {
    setUp(() {
      service = const ColorHexesApiService(
  ...
  group("Given a $ColorHexesApiService with a valid apiIndex", () {
    setUp(() {
      final apiIndex = ApiIndex.fromJsonEntries(testIndex);
      final responseBody = jsonEncode(testColorHexesApiResponse);
      service = ColorHexesApiService(
    ...
    group("when baseUri is called", () {
      test("then the request uri is properly built", () {
        expect(
          ...
          contains("https://test.com/colors/search/hexes"),
        );
      });
    });
  ...
  group("Given a $ColorHexesApiService with a failing http client", () {
    setUp(() {
      service = const ColorHexesApiService(
